...
def df_info(df):
    ...
    df_info_dataframe = pd.DataFrame(data={'Column': names, 'Non-Null Count': nn_count, 'Data Type': dtype})
...
def sidebar_multiselect_container(message, arr, key):
    ...
    if select_all_button:
        selected_cols = container.multiselect(message, arr, default=list(arr))
    else:
        selected_cols = container.multiselect(message, arr, default=arr[0])
...
def drop_column(df, column):
    if column in df.columns:
        ...
        print(f"Column '{column}' successใ")
    else:
