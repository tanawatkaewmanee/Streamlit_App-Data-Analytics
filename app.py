...
st.set_page_config(layout="wide", page_title='Streamlit for Data Analysis')

st.header("Exploratory Data Analysis")

st.write('<p style="font-size:160%">You will be able to✅:</p>', unsafe_allow_html=True)

st.write('<p style="font-size:100%">&nbsp 1. See the Dataset</p>', unsafe_allow_html=True)
st.write('<p style="font-size:100%">&nbsp 2. Open View column names, data types info</p>', unsafe_allow_html=True)
st.write('<p style="font-size:100%">&nbsp 3. Open View the count and percentage of NA values</p>', unsafe_allow_html=True)
st.write('<p style="font-size:100%">&nbsp 4. Drop Column</p>', unsafe_allow_html=True)
...
if dataset:
    if file_format == 'csv' or file_format == 'excel':
        if file_format == 'csv':
        ...
        all_vizuals = ['Info', 'NA Info', 'Descriptive Analysis', 'Target Analysis',
                       ...
                       'Box Plots', 'Outlier Analysis', 'Variance of Target with Categorical Columns', 'Drop Column']
        ...
        if 'Drop Column' in vizuals:
            if st.button('Drop Column'):
                column_to_drop = st.text_input('Enter column name:')
                df = functions.drop_column(df, column_to_drop)
                st.dataframe(df)
